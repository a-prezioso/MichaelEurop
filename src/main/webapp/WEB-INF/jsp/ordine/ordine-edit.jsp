...
<head>
	...
	<title>OrdineAcquisto Edit</title>
...
<body>
<spring:url value="${urlOrdine}" var="backUrl"/>
...
<sec:authorize access="isAuthenticated()">
	...
	<div class="container">
		<spring:url value="/ordine/save" var="saveURL" />
		
		<c:choose>
			<c:when test="${ordineAcquistoForm.chiaveOrdineAcquisto>0}">
				<h2>Modifica Ordine d'Acquisto</h2>
			</c:when>
			<c:otherwise>
				<h2>Nuovo Ordine d'Acquisto</h2>
			</c:otherwise>
		</c:choose>
		
		<c:choose>
			<c:when test="${fornitoreList.size()>0}">
				<form:form modelAttribute="ordineAcquistoForm" method="post" action="${saveURL}" cssClass="form" id="formId">
					<form:hidden path="chiaveOrdineAcquisto" />
					<div class="form-group">
						<label>Ordine d'Acquisto</label>
						<form:input path="ordineAcquisto" cssClass="form-control" />
					</div>
					<div class="form-group">
						<label>Numero</label>
					...
					<div class="form-group" >
						<label>Data</label>
						<fmt:formatDate value="${ordineAcquistoForm.data}" pattern="dd/MM/yyyy" var="fmtData"/>
		        		<input class="form-control" name="data" placeholder="dd/MM/yyyy" type="text" value="${fmtData}" data-provide="datepicker"/>
					</div>
					<div class="form-group">
						<label>Fornitore</label>
						<form:select path="fornitore" cssClass="form-control" onfocus='this.size=5;' onblur='this.size=1;' onchange='this.size=1; this.blur();'>
					...
					<h4 align="center">Dettagli</h4>
						...
						<input type="submit" value="Nuovo" onclick="setAction('/ordine/dettaglio/add');" class="btn btn-primary"/>
								...
								<table class="table table-striped table-bordered mydatatablenosearch" style="width:100%">
									...
									<tbody>
										...
										<c:forEach items="${dettagli}" var="currentDettaglio" varStatus="loop">
											...
											<c:set var="totImporto" value="${totImporto + (currentDettaglio.quantita * currentDettaglio.importoUnitario)}"/>
										</c:forEach>
									</tbody>
				...
				<!-- 			</div> -->
								...
								<br>
								<input type="submit" value="Modifica" onclick="return setUrl('/ordine/dettaglio/update/');" class="btn btn-warning"/>
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<input type="submit" value="Elimina" onclick="return setUrl('/ordine/dettaglio/delete/');" class="btn btn-danger"/>
